    ...
    public class Calculator
    {
        ...
        public DataTable GetShedule()
        {
           ...
           /* //расчёты ниже
            ...
            static double CalculateAnnuityPayment(double loanAmount, int months, double procent)
            {


                double monthlyInterestRate = procent / 12; // ставка

                double monthlyPayment = loanAmount * (monthlyInterestRate + (monthlyInterestRate / (Math.Pow((1 + monthlyInterestRate), months) - 1)));
            ...
            static void GenerateDifferentiatedPayments(double loanAmount, int months, double procent)
            {
                double monthlyInterestRate = procent / 12; // ставка 
                StreamWriter sw = new StreamWriter("C:\\Users\\marse\\Desktop\\text.txt", false);

                for (int month = 1; month <= months; month++)
                {
                    int monthNum = month % 12;
                    string monthName = "";
                    int year = 2024 + (int)Math.Truncate((decimal)(month / 12));
